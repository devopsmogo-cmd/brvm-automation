        ...
        try:
            ...
            if propositions:
                ...
                <table style="width:100%; margin: 10px 0;">
                    <tr><th style="background: #1F4E78; color: white; padding: 8px;">Recommandation</th><th style="background: #1F4E78; color: white; padding: 8px;">Nombre</th><th style="background: #1F4E78; color: white; padding: 8px;">Secteurs</th></tr>
                    <tr>
                ...
                # Ajouter secteurs ACHETER
                acheter_secteurs = [p.get('secteur', 'N/A') for p in propositions if p.get('recommandation') == 'ACHETER']
                synthese_propositions_html += ", ".join(acheter_secteurs) if acheter_secteurs else "—"
                synthese_propositions_html += f"""
                        </td>
                    </tr>
                    <tr>
                ...
                # Ajouter secteurs VENDRE
                vendre_secteurs = [p.get('secteur', 'N/A') for p in propositions if p.get('recommandation') == 'VENDRE']
                synthese_propositions_html += ", ".join(vendre_secteurs) if vendre_secteurs else "—"
                synthese_propositions_html += f"""
                        </td>
                    </tr>
                    <tr>
                ...
                # Ajouter secteurs CONSERVER
                conserver_secteurs = [p.get('secteur', 'N/A') for p in propositions if p.get('recommandation') == 'CONSERVER']
                synthese_propositions_html += ", ".join(conserver_secteurs) if conserver_secteurs else "—"
                synthese_propositions_html += f"""
                        </td>
                    </tr>
                    <tr style="background: #e8f4f8;">
                        <td><strong>Actions impactées</strong></td>
                        <td colspan="2"><strong>{synthese_prop.get('actions_impactees_total', 0)}</strong></td>
                    </tr>
                </table>
                <p><strong>Détail par secteur:</strong></p>
                <ul style="margin: 10px 0;">
                """
                
                # Afficher chaque proposition avec détails
                for prop in propositions[:5]:
                    ...
                    color = '#28a745' if rec == 'ACHETER' else '#dc3545' if rec == 'VENDRE' else '#ffc107'
                    
                    actions_str = ", ".join(actions[:4]) if actions else "—"
                    synthese_propositions_html += f'<li><strong style="color: {color};">{secteur}: {rec}</strong> ({conf}) - Actions: {actions_str}</li>'
